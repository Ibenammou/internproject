...
def send_whatsapp_message(doctor_phone_number, message):
    now = datetime.datetime.now()
    send_time_hour = now.hour
    send_time_minute = now.minute + 2  # Send after 2 minutes from now
    kit.sendwhatmsg(doctor_phone_number, message, send_time_hour, send_time_minute)
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        if best_match_idx is not None:
            ...
            message = f"Patient {patient['name']} is here for the {patient['service']} service. Can they enter now, or should they wait? Please reply with the wait time in minutes or 'Enter'."
            send_whatsapp_message('+212676687703', message)  # Use international format with '+' and country code
